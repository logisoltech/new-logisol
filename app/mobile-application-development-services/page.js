...
const Page = () => {
  ...
  const paragraphs = [
    {
      heading: 'Native & Cross-Platform Mobile Solutions',
      text: (
        <>
          At <span className="text-cyan-400 font-semibold">Logisol Technologies</span>, we develop
          powerful mobile applications for iOS and Android. Our expert team creates native and
          cross-platform apps that deliver exceptional user experiences.
        </>
      ),
    },
    {
      heading: 'Seamless Performance Across All Devices',
      text: (
        <>
          We leverage React Native, Flutter, and native technologies to build apps that perform
          flawlessly. From UI/UX design to backend integration, we ensure your app works perfectly
          on every device.
        </>
      ),
    },
    {
      heading: 'From Concept to App Store Launch',
      text: (
        <>
          Our mobile app development services include prototyping, development, testing, and
          deployment. We handle everything from initial design to App Store and Play Store submissions,
          ensuring a smooth launch process.
        </>
      ),
    },
    {
      heading: 'Scalable Apps That Grow With Your Business',
      text: (
        <>
          Partner with us to create mobile applications that scale with your business needs. Let&apos;s
          build apps that engage users and drive real business value.
        </>
      ),
    },
  ];
  ...
  return (
    <main className="text-white relative">
      ...
      <div className="flex flex-col items-center justify-center h-[20rem] sm:h-[25rem] lg:h-[30rem] px-4">
        <h1
          className={`${headingFont.className} font-extrabold text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl text-center text-white w-full max-w-4xl mx-auto px-4 sm:px-6`}
          style={{
            textShadow: '0 0 40px rgba(255, 255, 255, 0.3), 0 0 80px rgba(255, 255, 255, 0.2)',
          }}
        >
          MOBILE APPLICATION <span className="text-cyan-400">DEVELOPMENT</span> SERVICES
        </h1>
        
        {/* Trust Badges */}
        <div className="flex items-start justify-center gap-4 sm:gap-6 mt-6 sm:mt-8 flex-wrap">
          <img 
            src="/clutch.png" 
            alt="Clutch" 
          ...
          <div className="flex flex-col items-start gap-1">
            ...
            >
              <img src="/trust.png" alt="Trustpilot" className="h-8 sm:h-10 lg:h-14 w-auto object-contain opacity-80 group-hover:opacity-100 transition-opacity" />
            </a>
{/* <div
      ...
      <Portfolio defaultTab="Mobile Development" />
      <CaseStudy />
      <Pricing defaultTab="App Development" hideTabs={true} />
